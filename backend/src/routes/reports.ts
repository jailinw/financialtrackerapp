...
router.get('/expenses.csv', requireAuth, async (req: AuthRequest, res) => {
  const { start, end } = req.query;
  const result = await pool.query(
    `SELECT occurred_at, description, amount_cents, fee_cents, net_cents, category, tags
     FROM transactions
     WHERE user_id = $1
...
router.get('/quarterly.pdf', requireAuth, async (req: AuthRequest, res) => {
