...
router.post('/stripe', requireAuth, async (req: AuthRequest, res) => {
  const userId = req.user!.id;
  const conn = await pool.query('SELECT stripe_account_id, access_token FROM stripe_connections WHERE user_id = $1', [userId]);
  if (!conn.rowCount) return res.status(400).json({ error: 'Stripe not connected' });
  ...
  for (const charge of charges.data) {
    ...
    await pool.query(
      `INSERT INTO transactions (user_id, stripe_id, type, amount_cents, fee_cents, net_cents, currency, description, occurred_at, metadata)
      ...
      [userId, charge.id, charge.amount, fee, charge.amount - fee, charge.currency, charge.description, charge.created, JSON.stringify(charge)]
    );
  }
