...
router.get('/', requireAuth, async (req: AuthRequest, res) => {
  ...
  const result = await pool.query(
    `SELECT id, stripe_id, type, amount_cents, fee_cents, net_cents, currency, description, occurred_at, category, tags
...
router.patch('/:id', requireAuth, async (req: AuthRequest, res) => {
  ...
  const updated = await pool.query(
    `UPDATE transactions SET category = $1, tags = $2::jsonb WHERE id = $3 AND user_id = $4 RETURNING *`,
    [nextCategory, JSON.stringify(nextTags), id, req.user!.id]
  );
