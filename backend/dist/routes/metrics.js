...
router.get('/', requireAuth, async (req, res) => {
    ...
    const txResult = await pool.query(`SELECT
     ...
     FROM transactions
     WHERE user_id = $1 AND occurred_at >= $2`, [userId, start]);
    const profileResult = await pool.query('SELECT federal_rate, state_rate, self_employment_rate FROM tax_profiles WHERE user_id = $1', [userId]);
